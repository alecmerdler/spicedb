...
// NewNatsDatastore creates a new Datastore compliant datastore backed by memdb.
//
// If the watchBufferLength value of 0 is set then a default value of 128 will be used.
func NewNatsDatastore(
...
func (mdb *natsDatastore) SnapshotReader(dr datastore.Revision) datastore.Reader {
	mdb.RLock()
	defer mdb.RUnlock()

	if len(mdb.revisions) == 0 {
		return &natsReader{nil, nil, fmt.Errorf("memdb datastore is not ready"), time.Now()}
	}

	if err := mdb.checkRevisionLocalCallerMustLock(dr); err != nil {
		return &natsReader{nil, nil, err, time.Now()}
	}
	...
	if rev.db == nil {
		return &natsReader{nil, nil, fmt.Errorf("memdb datastore is already closed"), time.Now()}
	}
	...
	return &natsReader{noopTryLocker{}, txSrc, nil, time.Now()}
}

func (mdb *natsDatastore) SupportsIntegrity() bool {
	return true
}

func (mdb *natsDatastore) ReadWriteTx(
	ctx context.Context,
	...
	for i := 0; i < txNumAttempts; i++ {
		...
		rwt := &natsReadWriteTx{natsReader{&sync.Mutex{}, txSrc, nil, time.Now()}, newRevision}
...
func (mdb *natsDatastore) ReadyState(_ context.Context) (datastore.ReadyState, error) {
	mdb.RLock()
	defer mdb.RUnlock()
...
func (mdb *natsDatastore) OfflineFeatures() (*datastore.Features, error) {
...
func (mdb *natsDatastore) Features(_ context.Context) (*datastore.Features, error) {
	return mdb.OfflineFeatures()
}

func (mdb *natsDatastore) Close() error {
	mdb.Lock()
	defer mdb.Unlock()
