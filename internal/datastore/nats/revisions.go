...
func (mdb *natsDatastore) newRevisionID() revisions.TimestampRevision {
	mdb.Lock()
	defer mdb.Unlock()
...
func (mdb *natsDatastore) HeadRevision(_ context.Context) (datastore.Revision, error) {
	mdb.RLock()
	defer mdb.RUnlock()
	if mdb.db == nil {
...
func (mdb *natsDatastore) SquashRevisionsForTesting() {
...
func (mdb *natsDatastore) headRevisionNoLock() revisions.TimestampRevision {
	return mdb.revisions[len(mdb.revisions)-1].revision
}

func (mdb *natsDatastore) OptimizedRevision(_ context.Context) (datastore.Revision, error) {
	mdb.RLock()
	defer mdb.RUnlock()
	if mdb.db == nil {
		return nil, fmt.Errorf("datastore has been closed")
...
func (mdb *natsDatastore) CheckRevision(_ context.Context, dr datastore.Revision) error {
	mdb.RLock()
	defer mdb.RUnlock()
	if mdb.db == nil {
...
func (mdb *natsDatastore) checkRevisionLocalCallerMustLock(dr datastore.Revision) error {
	now := nowRevision()
...
func (mdb *natsDatastore) revisionOutsideGCWindow(now revisions.TimestampRevision, revisionRaw datastore.Revision) bool {
